...
search_mod = load(ROOT / "templates/codex/scripts/memory_search.py", "memory_search")
context_mod = load(ROOT / "templates/codex/scripts/memory_context.py", "bf_memory_context")
usage_mod = load(ROOT / "templates/codex/scripts/memory_usage.py", "memory_usage")
router_mod = load(ROOT / "templates/codex/scripts/memory_router.py", "memory_router")
...
class NativeMemorySyncTests(unittest.TestCase):
    ...
    def test_new_machine_restores_newer_whole_remote_snapshot_without_backup(self) -> None:
        with tempfile.TemporaryDirectory() as raw:
            ...
            self.assertFalse((base / ".local-memories.bridgeforge-replaced").exists())
