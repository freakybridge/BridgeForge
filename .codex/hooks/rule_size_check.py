...
机制:
1. PostToolUse: 仅对编辑 `.codex/rules/*.md` 的 Edit/Write 触发, 读工作树内容跑 check_rule,
   跨阈值输出 [rule-size] 警告到 stdout(非阻塞)。
2. pre-commit: 对 `git diff --cached` 命中的 `.codex/rules/*.md`(排除 meta_rule_design.md),
...
详见 .codex/rules/meta_rule_design.md。
"""
...
# 工作流横切 doc、架构/模块常驻、可移植性横切 .codex/config/libs。豁免触发器宽度检查，
    ...
    if name not in CROSS_CUTTING_RULES:
        ...
        if broad:
            violations.append(
                ...
                f"收紧到具体子目录（≥2 段前缀如 a/b/**）或真红线进 AGENTS.md"
            )
...
def _staged_rule_files() -> list[str]:
    """git diff --cached 命中的 .codex/rules/*.md(排除 meta_rule_design.md)。"""
    try:
        r = subprocess.run(
...
def pre_commit() -> int:
    """pre-commit 硬拦: staged .codex/rules/*.md 读 staged blob 跑 check_rule。
...
def main() -> int:
    ...
    # 只关心 .codex/rules/*.md
    p = Path(file_path)
    if not (".codex" in p.parts and "rules" in p.parts and p.suffix == ".md"):
        return 0
    ...
    for v in violations:
        print(f"[rule-size]   - {v}")
    print("[rule-size] 详见 .codex/rules/meta_rule_design.md §5 量化红线 + §6 维护节奏")
    return 0
