#!/usr/bin/env python3
"""只读检查当前平台的 BridgeForge 托管 skill 是否与本地账本一致。

本 hook 不访问 GitHub、本地 BridgeForge clone、~/.bridgeforge 或 ~/.agents。
发现托管账本缺失、损坏、skill 缺失或内容漂移时，只提示运行无参
`/bridgeforge`；SessionStart 期间绝不自动修复。
"""
...
def _warn(detail: str) -> None:
    print(f"[skill-sync] {detail}。请运行无参 /bridgeforge 重新同步。")
...
def main() -> None:
    ...
    entry = shelf / "bridgeforge" / "SKILL.md"
    ledger_path = platform_root / "bridgeforge-managed.json"

    if not entry.is_file() and not ledger_path.exists():
        return
    if not ledger_path.is_file():
        _warn("BridgeForge 托管账本缺失")
        return
    try:
        ledger = json.loads(ledger_path.read_text(encoding="utf-8-sig"))
    except (OSError, json.JSONDecodeError):
        _warn("BridgeForge 托管账本无法读取")
        return
    if not isinstance(ledger, dict):
        _warn("BridgeForge 托管账本格式无效")
        return
    if ledger.get("schema_version") != 1 or ledger.get("platform") != platform:
        _warn("BridgeForge 托管账本版本或平台不匹配")
        return
    records = ledger.get("records")
    if not isinstance(records, dict) or not records:
        _warn("BridgeForge 托管账本没有有效记录")
        return
    ...
    for raw_name, record in records.items():
        ...
        if not current:
            stale.append(name)
    if invalid:
        _warn("BridgeForge 托管账本包含无效 skill 记录")
    elif stale:
