---
description: Codex ctx-budget 口径：复用 Claude 成熟机制，但 Codex 窗口按 /status 实测 258K 校准，hook 用 transcript usage 计算比例。
---
...
- Codex 侧不能照抄 Claude 1M。Codex 官方手册只说明线程必须放入模型 context window、窗口因模型而异，且 `config.toml` 支持 `model_context_window`；手册没有确认当前 Codex Desktop + gpt-5.5 的 app 实际 compact 上限就是 1M。
- Codex `/status` 截图显示“背景信息：剩余 39%（已使用 158,829 / 共 258K）”，因此本轮把 Codex ctx-budget 默认窗口校准为 `DEFAULT_CODEX_WINDOW = 258_000`。
