...
1. 新增根目录 `shared-skill-manifest.json`、Windows 首次安装脚本和可恢复的共享 skill updater。manifest 将 `bridgeforge` 作为平台托管 command bundle：除入口 `SKILL.md` 外，分发它所需的 `references/`、`templates/`、项目迁移脚本和 updater；其他通用 skill 按各自目录分发。
