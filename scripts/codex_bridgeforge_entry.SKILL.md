...
4. 确认该目录同时包含 `SKILL.md`、`references/`、`templates/` 与 `scripts/bridgeforge_shared_update.ps1`，然后读取其中的 `SKILL.md`，按其说明继续。无参数调用原样转交；`switch codex` 转交时必须把当前宿主固定声明为 `codex`，并保证底层调用包含 `--current-host codex`。当前宿主不得取自用户输入，也不得省略。
