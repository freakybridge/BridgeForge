...
| Skill | 触发 | 用途 |
|------|------|------|
| `confirm` | Claude `/confirm`；Codex `$confirm` | 统一单题选择式需求确认，落盘确认卡后交接 develop / debate / collab |
| `develop` | Claude `/develop`；Codex `$develop` | 大需求交付流水线：澄清需求、落盘文档、自动开发、独立验证、用户试用反馈闭环 |
...
| `skills/*/SKILL.md` | **通用** | 协作 skill 集（confirm / develop / plan / collab / debate / escalate / snapshot / resume / git-sync / archive-scan / todo / find-doc / find-memory / summary / sync-docs / harvest / spinoff / focus）。Step 0 自检补齐到 `$USER_SKILLS_DIR/` 平级；Step 0.5 清掉项目里的重复副本（单一源）。find-doc / sync-docs 的项目专属字典外置 `$PROJECT_AGENT_DIR/*.map.md`（不进 skill 本体）。 |
