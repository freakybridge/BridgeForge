...
```bash
git -C "$HOME/.claude/skills/bridgeforge" pull --ff-only
```

- pull 失败（冲突 / 网络）→ 报告并停下，**不强拉**；让用户先手动处理 clone 再重跑。
- clone 不是 git 仓库（用户手动拷的）→ 跳过 pull，提示"建议改用 `git clone` 以便后续 `/bridgeforge` 自动拉更新"。
...
- 为什么这是第一闸：源头仓库**故意不带** `.claude/.bridgeforge_version`（靠 `templates/` 存在性识别身份，比版本戳更本质，见 [docs/design-rationale.md](docs/design-rationale.md) §9.4）。没有这道闸，源头会落进下方"无戳 + 有 CLAUDE.md"分支被误当 init → 等于装修队拆自己样板间。

**Step 前-3 认场子（判定模式）**：

```bash
test -f .claude/.bridgeforge_version && cat .claude/.bridgeforge_version
```
...
| 场景 | 判据 | 走哪 |
|------|------|------|
| **更新** | 有 `.claude/.bridgeforge_version` | 【更新模式】跳到下方 "## 更新模式" 段（Step 0 + Step 0.5 自检仍先跑一遍：幂等补 skill + 清项目重复副本） |
| **收编 (adopt)** | 无戳，但已有 CLAUDE.md/rules **像 bridgeforge 铺过的**（指纹 ≥2 项命中） | 【收编模式】跳到下方 "## 收编模式 (adopt)" 段 |
...
本 skill 仓库内的 `skills/` 子目录包含全部通用协作 skill（脱敏自 StratusAgent 长期沉淀；清单以目录实际内容为准，**不再硬编码个数**）。Claude Code 只扫描 `~/.claude/skills/<name>/SKILL.md` 顶层，**不递归**——所以 `~/.claude/skills/bridgeforge/skills/<name>/SKILL.md` 这样嵌套的 skill 扫不到，必须复制到 `~/.claude/skills/<name>/` 平级。

**自检逻辑（幂等：缺失→装 / 旧版→提议更新 / 定制→问，绝不静默覆盖）**：

```bash
SKILL_DIR="$HOME/.claude/skills/bridgeforge"   # 本 skill 的仓库 clone 位置
...
| `harvest` | `/harvest` | 把下游攒的通用经验脱敏反哺回 bridgeforge 上游（无参批量清收件箱 / 带描述立即单条） |
...
> **背景**：通用协作 skill 的**单一源是 bridgeforge**，装到**用户级** `~/.claude/skills/`。若项目自己的 `.claude/skills/` 里也有同名副本 → **shadow 掉用户级单一源**：改进流不到项目、各项目副本各自漂移。本步把重复副本清掉，让通用 skill 一律从用户级解析。
>
...
对每个 bridgeforge 出品的通用 skill（清单 = `skills/` 目录），若 `<project>/.claude/skills/$s/` 存在就分类：

```bash
SKILLS_SRC="$HOME/.claude/skills/bridgeforge/skills"
...
> 注：若该 dir 有 `.claude/.bridgeforge_version`，前置步骤已路由到**更新模式**，不会到这里。本分支只针对"有 CLAUDE.md 但无版本戳"的非托管项目。
...
模板根在**本 skill 目录**的 `templates/`。skill 安装后通常在 `~/.claude/skills/bridgeforge/templates/`。
...
```gitignore
# === bridgeforge 协作骨架机制自动生成（勿提交，由 /bridgeforge 维护）===
__pycache__/
*.pyc
...
5. **提示用户**：
   ...
   - `skill_sync_check` SessionStart hook 自动启用 → 用户级通用 skill 与上游漂移时，session 开始会打印 `[skill-sync]` 提示跑 `/bridgeforge` 同步（只读检测，不自动改）
...
>
> **请先装 Python（≥ 3.8）再重跑 `/bridgeforge`**：
...
**先写版本戳**（让本项目纳入更新管理）：把本 skill clone 的 `VERSION` 内容写进 `<project>/.claude/.bridgeforge_version`：

```bash
cp "$HOME/.claude/skills/bridgeforge/VERSION" .claude/.bridgeforge_version
```

之后用户在本项目重跑 `/bridgeforge` 即自动进入**更新模式**（拉上游增量），不会再全新铺设。
...
> 触发：前置 Step 前-3 判定 cwd 无 `.claude/.bridgeforge_version`，但已有 CLAUDE.md/rules 命中 bridgeforge 衍生指纹（≥2 项）。常见于 **pre-stamp 老安装**（版本戳机制 v0.14.0 才引入）或手动拷过模板的项目。
>
...
2. 用户确认后写戳：
   ...
   cp "$HOME/.claude/skills/bridgeforge/VERSION" .claude/.bridgeforge_version
   ```
   > 戳值 = 上游**当前** VERSION，等于声明"以此为同步基线"。首次收编**不补历史增量**（视现状为最新），从下次 `/bridgeforge` 起才按 `(此版, 新版]` 拉 `[product]` 增量。
...
> 触发：前置步骤检测到 `.claude/.bridgeforge_version`。这是 `/bridgeforge` 的"拉上游增量到现有项目"流程——**机械半场**（拉 / diff / 分类 / 呈现）由 skill 做，**判断半场**（rules/CLAUDE.md 选择性吸收）全程交用户。完整判据见 `~/.claude/skills/bridgeforge/docs/sync-from-upstream-playbook.md`（下称 playbook）。

### U1：算增量
- 读 cwd 的 `.claude/.bridgeforge_version`（下游上次同步的版本）vs clone 的 `VERSION`（上游当前版本，前置步骤已 pull 到最新）。
...
- 把 `.claude/.bridgeforge_version` 写成 clone 的当前 `VERSION`。
...
| `.claude/.bridgeforge_version` | 生成（非模板文件）| init 末尾写 = 安装时 bridgeforge 版本；重跑 `/bridgeforge` 据此进**更新模式**（拉上游 `[product]` 增量）而非全新铺设 |
