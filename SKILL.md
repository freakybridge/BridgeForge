...
**处理（堵数据丢失，复用 Step 0「不一致 → 问」的好心）**：

| 情况 | 动作 |
|------|------|
| **DUP-IDENTICAL**（与上游字节一致）| 直接删 `<project>/.claude/skills/$s/`，告知"已删重复副本，改从用户级单一源解析"。 |
| **DUP-DIVERGENT 但只是旧版镜像**（无项目专属内容，纯过时）| 同上删；删的是过时副本，用户级是更新版。 |
| **DUP-DIVERGENT 且含项目专属数据** | **绝不静默删**。两类典型：① `find-doc` / `sync-docs` 旧版**内联字典/映射表**（外置前的填充版）→ 先把字典迁到 `.claude/find-doc.map.md` / `.claude/sync-docs.map.md`（格式分别见 find-doc Step 4 / sync-docs Step 7），迁完再删副本；② 其他被用户改过的定制 → 给 `diff` 问用户"删（回落用户级单一源）/ 保留定制"。 |
