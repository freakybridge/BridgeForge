...
## 根因（一个根，四处症状）
中文文本 + **中文版 Windows 默认 GBK** + 工具按 UTF-8 读 = 乱码 U+FFFD（Unicode 替换字符）。全部 transcript 扫出 **8.5 万个 U+FFFD**、横跨所有下游项目（StratusAgent / CausisRiskSuite / ClaudeBridgeAssist）。
...
- **模型自身流式解码抽风**（junction 那次出现的 U+FFFD，在 assistant 输出、客户端层）——harness 够不着，任何大模型都有小概率，**不可防**。见 [[feedback-review-technique]] 式的"看到跑偏就打断纠偏"。
