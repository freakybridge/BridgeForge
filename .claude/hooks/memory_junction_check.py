...
把 memory 纳入项目 git（项目内 `.claude/memory/`），但 Claude Code 读写走系统路径
`~/.claude/projects/<project-hash>/memory/`。本 hook 在每次 session 开始时静默检查
...
可移植：项目无关。repo root 取自 hook 自身路径（`.claude/hooks/` 上两级），
project-hash 按 Claude Code 编码规则（路径分隔符 + `:` → `-`，Windows 盘符小写）从 repo root 推导。
"""
...
def _project_hash(root: Path) -> str:
    """按 Claude Code 编码规则把绝对路径转 project-hash。

    实测规则（从 `~/.claude/projects/` 下真实目录名反推）：**每个非字母数字字符替换为
    ...
    而 Claude Code 建目录时用的是启动 cwd 的原始大小写（可能是小写 `d`）。二者大小写
...
def _system_memory_path() -> Path | None:
    """推导系统 memory 路径；父目录 `~/.claude/projects/<hash>/` 不存在则返 None（不瞎建）。"""
    h = _project_hash(REPO_ROOT)
    proj_dir = Path.home() / ".claude" / "projects" / h
