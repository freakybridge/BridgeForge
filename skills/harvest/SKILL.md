---
name: harvest
description: 把下游项目攒下的通用经验脱敏后反哺回 setup_agent 上游骨架库（reverse-sync）。无参=读收件箱批量处理；带一句描述=立即单条快车道。
user-invocable: true
argument: 可选——要立即反哺的一条经验描述。带参走快车道（跳过收件箱）；不带参读收件箱批量处理。
---

# /harvest — 通用经验反哺上游 setup_agent

把当前下游项目攒下的**通用**经验，脱敏后写回 setup_agent 骨架库的 `templates/` / `skills/`，让所有项目受益。
...
setup_agent 上游仓库通常在 `~/.claude/skills/setup_agent/`（clone 或 symlink）。harvest 的写入目标是它的 `templates/` / `skills/`。
- 找不到上游 clone → 停下，提示用户先 clone 再跑。
- 上游是 git 仓库 → 后续改动**不自动 commit/push**，留给用户。

## 仪式（严格按 reverse-sync-playbook 执行）

权威流程 + 脱敏 checklist 见 `~/.claude/skills/setup_agent/docs/reverse-sync-playbook.md`。核心步骤：
