...
`.codex/skill-routing.json` 是 18 个下游通用 skill 的路由单一事实源；`bridgeforge` 与 `create-worktree` 是用户级全局入口，只在该文件的 `global_entries` 中单列，不属于这 18 个项目 skill，且必须由主对话执行、禁止再拆给子 agent。
...
- 用户显式 `$git-sync` 时，主对话完成只读审查后直接且只运行当前宿主的 `.<host>/scripts/codex_git_sync.py`；禁止拆成手工 Git 命令或再次委派，分叉、冲突、失败和任何决策仍留在主对话。
...
memory 纳入项目 git（`.codex/memory/`），与 Codex 原生 `~/.codex/memories/` 完全分离，禁止建立 `~/.codex/projects/**/memory/` junction。`SessionStart` 必须先重建索引，再注入最多 6000 字符的 `MEMORY.md`；`UserPromptSubmit` 自动给出 3-5 个候选，正文成功读取后才记录 used 回执。
