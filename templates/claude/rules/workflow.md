...
Python 项目装有 `.claude/hooks/version_check.py`（PreToolUse / Bash）：`git commit` 前 staged **必须**含根 `VERSION`；仅暂存业务 manifest 不算版本 bump，直接 `exit 2` 阻断。豁免：message 加 `[skip-version]` / `--amend` / 正在 merge。非 Python 项目无此 hook，退化为仅靠 §9 软规则（自觉 + 收尾自查 §4）。
