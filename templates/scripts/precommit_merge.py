...
MANAGED_BEGIN = b"# >>> BRIDGEFORGE_CODEX_MANAGED_BEGIN"
MANAGED_END = b"# <<< BRIDGEFORGE_CODEX_MANAGED_END"
...
LEGACY_MANAGED_BEGIN = b"# >>> BRIDGEFORGE_MANAGED_BEGIN"
LEGACY_MANAGED_END = b"# <<< BRIDGEFORGE_MANAGED_END"
...
def build_plan(project_root: Path, template_path: Path) -> MergePlan:
    ...
    parse_before = before
    if (
        before.count(LEGACY_MANAGED_BEGIN) == 1
        and before.count(LEGACY_MANAGED_END) == 1
        and MANAGED_BEGIN not in before
        and MANAGED_END not in before
    ):
        parse_before = before.replace(
            LEGACY_MANAGED_BEGIN,
            MANAGED_BEGIN,
            1,
        ).replace(LEGACY_MANAGED_END, MANAGED_END, 1)
    try:
        target_layout = parse_layout(parse_before, target)
    except MergeBlocked:
    ...
    extension = parse_before[
        target_layout.extension_body_start:target_layout.extension_body_end
    ]
    after = (
        parse_before[:target_layout.managed_start]
        + template[template_layout.managed_start:template_layout.managed_end]
        + parse_before[target_layout.managed_end:]
    )
    return MergePlan(
        target,
        before,
