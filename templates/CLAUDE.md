...
<!-- 项目专属红线（3-5 条"必须 X / 禁止 Y"硬约束），填好后删本注释。
照下面三类各想一条，没有的留空；不确定就先写你最怕 Claude 踩的那一条，其余随开发增量补：
- 数据流方向：数据只能从 ? 流向 ?，禁止反向 / 跨层直连（例：UI 禁止直接写 DB，必须走 service 层）
- 容量 / 资源上限：? 不能超过 ?（例：ESP32 固件不能超过 Flash 容量；单次查询禁止全表拉取）
- 时序 / 调用约束：? 必须在 ? 内 / 之前（例：屏幕刷新必须在主 loop 内；写操作必须先校验再落盘） -->
...
<!-- 填项目的构建/测试/启动命令，填好后删本注释。照下表填，没有的行删掉；
不知写什么 → 回想你每天敲得最多的那几行终端命令：
| 动作 | 命令 |
|------|------|
| 构建 / 编译 | 例：cargo build ｜ npm run build ｜ 编译并烧录固件 |
| 运行 / 启动 | 例：python -m webapi ｜ npm run dev |
| 测试 | 例：pytest ｜ cargo test ｜ npm test |
| 代码检查 | 例：ruff check ｜ cargo clippy ｜ eslint | -->
...
<!-- 列顶层目录及职责，帮 Claude 快速定位代码，填好后删本注释。
跑一次 `ls` 看顶层有哪几个文件夹，照下面格式逐个补一句职责（一行一个目录）：
- `目录名/` — 这里放什么 / 负责什么
例：
- `firmware/` — ESP32 固件源码（C/C++）
- `webapi/`   — Python REST 服务，对外接口
- `hardware/` — 电路图与引脚定义 -->
