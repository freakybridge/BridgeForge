...
被两个 hook 复用：
- UserPromptSubmit：每次用户提交 prompt 前运行，输出注入到 Claude 上下文（让 Claude 实时
  感知 dirty 状态 / 远端漂移）
- SessionStart：session 开始时运行，让 Claude 一眼看到当前仓库状态 + snapshot 接续提示
  + 归档候选数
...
def _archive_hint() -> str:
    ...
    script = REPO_ROOT / ".claude" / "scripts" / "archive_scan.py"
